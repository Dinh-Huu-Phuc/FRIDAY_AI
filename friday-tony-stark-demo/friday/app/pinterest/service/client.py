"""Low-level client for Pinterest operations."""
...
from friday.app.pinterest.utils.helpers import build_resource_url, normalize_identifier


class PinterestClient:
    """Thin client that prepares low-level payloads for Pinterest."""
    ...
    def get_profile(self, request: GetProfileRequest) -> dict[str, object]:
        ...
        return {
            ...
            "label": f"Pinterest Profile {identifier}",
    ...
    def get_post_detail(self, request: GetPostDetailRequest) -> dict[str, object]:
        content_id = normalize_identifier(request.content_id or "pin")
        return {
            "id": content_id,
            "url": build_resource_url(self.settings.website_url, "pin", content_id),
            "label": f"Pinterest Pin {content_id}",
    ...
    def publish_content(self, request: PublishContentRequest) -> dict[str, object]:
        ...
        return {
            "id": f"draft-{draft_id}",
            "url": build_resource_url(self.settings.website_url, "pin", f"draft-{draft_id}"),
            "label": request.title or f"Pinterest Draft",
