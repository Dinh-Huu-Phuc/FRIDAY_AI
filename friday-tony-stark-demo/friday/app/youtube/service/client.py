"""Low-level client for YouTube operations."""
...
from friday.app.youtube.utils.helpers import build_resource_url, normalize_identifier


class YouTubeClient:
    """Thin client that prepares low-level payloads for YouTube."""
    ...
    def get_profile(self, request: GetProfileRequest) -> dict[str, object]:
        ...
        return {
            "id": identifier,
            "url": build_resource_url(self.settings.website_url, "channel", identifier),
            "label": f"YouTube Channel {identifier}",
    ...
    def get_post_detail(self, request: GetPostDetailRequest) -> dict[str, object]:
        content_id = normalize_identifier(request.content_id or "video")
        return {
            "id": content_id,
            "url": build_resource_url(self.settings.website_url, "video", content_id),
            "label": f"YouTube Video {content_id}",
    ...
    def publish_content(self, request: PublishContentRequest) -> dict[str, object]:
        ...
        return {
            "id": f"draft-{draft_id}",
            "url": build_resource_url(self.settings.website_url, "video", f"draft-{draft_id}"),
            "label": request.title or f"YouTube Draft",
