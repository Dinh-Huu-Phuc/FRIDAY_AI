...
def _build_windows_launcher_runtime_hint(*, command: str, app_query: str, result: dict) -> str:
...
class FridayAgent(Agent):
    ...
    async def on_user_turn_completed(self, turn_ctx, new_message) -> None:
        ...
        if not social_open_result:
            windows_app_query = _extract_windows_app_query(refined_user_text)
            if windows_app_query:
                windows_open_response = await asyncio.to_thread(
                    open_windows_app,
                    query=windows_app_query,
                )
                windows_launcher_hint = _build_windows_launcher_runtime_hint(
