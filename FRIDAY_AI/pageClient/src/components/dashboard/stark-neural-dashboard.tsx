...
export function StarkNeuralDashboard() {
  ...
  const [coreColor, setCoreColor] = useState("#ffbd34")
  const canUsePortal = useSyncExternalStore(subscribeToClientSnapshot, getClientSnapshot, getServerSnapshot)
  const terminalRef = useRef<HTMLDivElement>(null)
  const t = copy[locale]
  const coreRgb = useMemo(() => hexToRgb(coreColor), [coreColor])

  function updateCoreRgb(channel: "red" | "green" | "blue", value: string) {
    const next = { ...coreRgb, [channel]: clampRgb(Number(value) || 0) }
    setCoreColor(rgbToHex(next.red, next.green, next.blue))
  }
  ...
  const overlay = (
    <div className={overlayActive ? "stark-ai-overlay is-active" : "stark-ai-overlay"} aria-hidden={!overlayActive}>
      <div className="stark-ai-core-stage">
        <div className="stark-sphere-formula" aria-hidden="true">
          x^2 + y^2 + z^2 = R^2
        </div>
        <div className="stark-core-color-panel" aria-label="AI core color controls">
          <label className="stark-core-swatch">
            <span>CORE COLOR</span>
            <input type="color" value={coreColor} onChange={(event) => setCoreColor(event.target.value)} />
          </label>
          <label>
            <span>R</span>
            <input type="number" min="0" max="255" value={coreRgb.red} onChange={(event) => updateCoreRgb("red", event.target.value)} />
          </label>
          <label>
            <span>G</span>
            <input type="number" min="0" max="255" value={coreRgb.green} onChange={(event) => updateCoreRgb("green", event.target.value)} />
          </label>
          <label>
            <span>B</span>
            <input type="number" min="0" max="255" value={coreRgb.blue} onChange={(event) => updateCoreRgb("blue", event.target.value)} />
          </label>
        </div>
  ...
  return (
    <main className={overlayActive ? "stark-dashboard is-core-open" : "stark-dashboard"}>
      <DashboardBackground coreColor={coreColor} paused={overlayActive} />
